...
x = 5
y = 2
x < 5 and y < 5  # returns False because it requires that both expressions be True, and the first is not
x < 5 or y < 5  # returns True because at least one expression is True
not(x < 5 or y < 5)  # returns False because it negates the value within parentheses
```
...
truthy, and `False` if the value is falsy. For example:

- Any non-empty string is "truthy" (e.g., `bool("dog")` is `True`, as is `bool("a")` or `bool("0")`)
- Any non-zero number is "truthy" (e.g., `bool(1)` is `True`, as is `bool(0.001)`)
- `0` (zero) is "falsy" (so `bool(0)` is `False`)
- `None` is "falsy" (so `bool(None)` is also `False`)
- The empty string `""` is "falsy" (so `bool("")` is `False`)
