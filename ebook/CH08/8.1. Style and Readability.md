...
method from the `textwrap` module included in the standard library — so you don't have to install
any external dependencies to use it.
...
We touched on type hints briefly in [Chapter 6.4 Data Classes](../CH06/6.4.%20Data%20Classes.md),
but we'll cover them in more detail here. Although Python is a dynamically typed language, optional
_type hints_ can help you catch errors and make your code more readable. Some popular packages
(e.g., FastAPI) all but require type hints for your variables in function and class definitions.
Type hints are denoted by a colon (`:`) and a type annotation after the variable name, but before
the equals sign (`=`) in an assignment. For example, `number: int = 1` is a type hint for the
variable `number` indicating that it is an integer, and it is then assigned the value 1.
