...
allows you to control your computer by typing _commands_ instead of using a mouse. While it might
...
- **Windows**: Command Prompt or PowerShell, but _please do not use Command Prompt_ for this course,
...
> [!TIP]
>
> ### Optional: Alternative terminals
>
...
| `ls`                | **L**i**s**t files and folders in the current directory          | `ls` (or `dir` in Command Prompt)                          |
...
> [!IMPORTANT]
>
> ### Key term: current directory (or working directory)
>
...
> path).
>
> You can check "where you are" (aka, your current or working directory) with the `pwd` command, and
> you can change it with the `cd` command.
...
| `rm -rf directory`      | Same as above, skipping confirmation (beware!)              |

> [!WARNING]
>
> ### Dangerous commands
> 
> The `rm -rf` command is especially dangerous. Quite famously, all of Pixar's work to
> create Toy Story 2 was almost completely deleted by this command.
