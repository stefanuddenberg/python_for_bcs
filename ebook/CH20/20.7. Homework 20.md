...
Create a folder called `HW20/` (or reuse your Chapter 20 project folder and rename it — but if you
do rename it, beware of issues with your environment, which you may need to recreate if you have
problems activating/using it). Inside of your project folder you should have:
...
- `templates/`
  - `survey.html`
  - `thank_you.html`
- `populate_db.py` (new: you will create this file yourself)
...
- generate at least 30 fake survey responses. For these fake survey responses:
  ...
  - compute the Big Five trait scores for each response using the same helper function you implemented
    in Part 3
  - create `SurveyResponse` objects and save them to the database (one for each fake response)
...
When we unzip the project, it should run _as-is_ using the command `uvicorn main:app --reload` or
`uv run uvicorn main:app --reload` when run from the project folder (you can assume that we will set
up the correct virtual environment and have the correct dependencies installed). The "run as-is"
requirement still stands: **If it does not run, then it will not be graded.**
