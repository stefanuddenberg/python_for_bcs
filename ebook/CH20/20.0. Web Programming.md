...
- Lastly, the **database** (in this case, a SQLite file stored on your computer) stores the data
  ...
  to use the database in [Chapter 20.5](../CH20/20.5.%20Setting%20Up%20the%20Database.md).

Below is a diagram showing how data flows through the experiment, from the client to the server and
eventually to the database. (There are some details in this diagram that will make more sense as you
go through the chapter, such as GET and POST requests.)
...
<!-- GITHUB-NAV-START -->
<!-- Recapitulating the diagram for the github version -->
```mermaid
flowchart LR
  subgraph Client["Client (participant browser)"]
    UI["HTML form (survey.html)"]
