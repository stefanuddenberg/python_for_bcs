...
> [!IMPORTANT]
>
> ### Update your files thoughtfully!
>
> In this chapter, you will often revise the same files (`main.py`, `database.py`, templates, and so
> on) as we add new features. Sometimes, this is as simple as copying the code blocks you see and
> replacing the file contents wholesale. Other times, you will need to modify existing code, taking
> pieces of the new code and pasting them in the appropriate place in the app, without deleting what
> came before. If you blindly keep copying and pasting new code onto the ends of your files, your
> app simply will not work. Pay attention to whether a code block is asking you to **replace** old
> code, **modify** an existing function, or **add** something genuinely new. When in doubt, compare
> the new snippet to what you already have and make sure your file still has one coherent version of
> the program rather than several half-old versions glued together. Be particularly aware of
> duplicated code, as when you have multiple declarations of the same function or variable, or
> multiple copies of the same imports. (Imports which, of course, should be at the top of the file
> and not randomly scattered throughout it.)
...
Or, if your terminal is complaining that `uvicorn` cannot be found, you can just throw a `uv run` in
front of it:
...
And again, if that isn't working, throw a `uv run` in front of it:
