...
`survey_form()` function, which returns the HTML form to the client). POST requests, on the other
hand, are used when the client wants to _send_ data to the server (as with our `submit_survey()`
function). Of course, in practice, POST requests often also return data themselves (like the "Thank
you" page returned by the `submit_survey()` function) so it can be a two-way street. HTML requests
come in a few other varieties, such as PUT, DELETE, and PATCH, but they won't be covered here. GET
and POST are all we need for now.
