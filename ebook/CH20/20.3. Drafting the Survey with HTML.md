...
- The `name="XXX"` attributes are the ways we can refer to the input fields on the server (XXX is,
  of course, just a placeholder here)
...
- GET requests are used to _retrieve_ data from the server (as with our `survey_form()` function,
  which returns the HTML form to the client).
- POST requests, on the other hand, are used when the client wants to _send_ data to the server (as
  with our `submit_survey()` function). Of course, in practice, POST requests often also return data
  themselves (like the "Thank you" page returned by the `submit_survey()` function) so it can be a
  two-way street.

We won't cover the other HTML verbs here, as GET and POST are all we need for now. However, if
you're curious, you can learn more about the other HTML verbs in the
[MDN documentation](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods).

Second, this route makes use of an `async` function. This type of function stands in contrast to
"synchronous" functions, which are the default type of function in Python, and the only type we have
written so far in this course.
...
the `await` keyword, you must define the function with the `async def` keywords.
...
As you're getting started, you may encounter issues. This section will be updated with common issues
faced by the class and their solutions, so if you run into an error, first check here, then contact
me/the TAs to get your issue solved and potentially added to the book.
