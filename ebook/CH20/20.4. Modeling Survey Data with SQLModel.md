...
Let's start by adding an input class for our survey responses. This will store all the  fields for
our survey response. 
...
Next, we will create an input model for the survey response. This will be the same as the base
class, but we will separate them conceptually for clarity. Often, in real-life applications, we will
have a separate input model for the survey response, and a separate output model for the survey
response.

```python
class SurveyResponseIn(SurveyResponseBase):
    """
    This is the input model for a survey response.
    In this case, it's the same as the base class, but
    we separate them conceptually for clarity.
    """
    pass
```
...
- `SurveyResponseIn` is _not_ a database table. Here it is redundant with the base class, but it can
  come in handy later.
