...
One thing to note is that when the default `"\n"` is used in a print statement, this forces the
print statement to occur immediately by your operating system. But if you change this, you can
sometimes see weird behavior, like even having print statements occurring out of order, because your
operating system decides on its own when to execute them. You can force it to print right away using
the `flush=True` keyword argument. If you are changing the `end` argument in a print statement, it
is good to use `flush` at the same time.
