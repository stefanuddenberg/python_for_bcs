...
2-5. Inside that function, create print statements answering the following questions:
    ...
    5. Run the simulation from book section 3.5 (you can run it on your own in a separate file from this one). 
       ...
       the spread of infection. Then, change some of the optional parameters and describe how that changes the
       result.
...
6) In the main function, create a "category_list" containing the names of the categories in the story above,
stored as strings.
...
response). But you can earn 10% extra credit on the assignment for a solution that is not hard-coded, and where 
the code would still work if we added new inputs.
"""
```
