...
Create a directory called `HW18/`. Within it, create a `texts/` directory and a `src/` directory.
...
(though you will need to change the `book_id_list`):
...
Next we want to see which of the books is more positive or negative in its polarity, and how that changes over the
course of the book, and how it varies by different subjects in the book. In `src/`, create a script called
`analyze_books.py`. Paste in the following code, and then complete as described. You may use other imports as needed,
but you should not use any other 3rd party libraries.
...
def clean_text_string(text_string):
	...
	- replace the hidden character `\ufeff` (this is an encoding character in project Gutenberg texts) with an empty string
	- replace all line breaks `\n` with spaces
	- use a regular expression to find any cases of 2 or more spaces, and replace them with a single space each.
		Make sure you do this last one *after* the previous one, so that the previous doesn't result in two spaces
